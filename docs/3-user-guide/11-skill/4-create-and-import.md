---
sidebar_label: '3.11.4 创建技能'
title: 3.11.4 创建技能
---

# 3.11.4 创建技能

技能工坊支持两种方式产生新技能：从模板创建（含 AI 辅助生成）、从外部 URL 导入（GitHub、NodeHub、普通网页）。这两种方式都可以帮你把操作经验整理成标准 SKILL.md。

![创建技能表单：填写技能说明、技能 ID 和 SKILL.md 内容](http://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/rdk_studio/zh/skill-create-form.png)

![从链接生成技能：粘贴 GitHub、NodeHub 或文档链接后生成草稿](http://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/rdk_studio/zh/skill-link-import.png)

## 创建流程（模板 + AI 辅助）

| 步骤 | 操作 |
|---|---|
| 1 | *技能工坊 → 技能中心 → 创建* 选择空模板（已预填头部字段与注释） |
...
AI 辅助生成的 SKILL.md 通常质量较好，但仍需要你审核：
...
| trigger 不要太宽泛 | 不要写 `trigger: 摄像头`——容易和很多摄像头话题冲突；用 `trigger: USB摄像头黑屏,hobot_usb_cam,code -6` 等具体场景 |
| 正文用步骤化 | `1. 2. 3.` 而不是大段散文，AI 解析更稳定 |
| 附完整命令 | 别写"看一下进程"，要写清楚具体命令、参数和要查的进程名 |
